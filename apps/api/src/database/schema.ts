...
export const atsPlatformEnum = pgEnum('ats_platform', [
  'greenhouse', 'lever', 'ashby', 'workday', 'unknown', 'none',
])
...
export const seniorityLevelEnum = pgEnum('seniority_level', [
  'intern', 'junior', 'mid', 'senior', 'staff', 'director', 'unknown',
])
...
export const companyTypeEnum = pgEnum('company_type', [
  'public', 'private', 'nonprofit', 'government', 'unknown',
])
...
export const discoverySourceEnum = pgEnum('discovery_source', [
  'yc_directory', 'wellfound', 'manual_seed', 'self_referral', 'github',
])
...
export const discoveryQueueStatusEnum = pgEnum('discovery_queue_status', [
  'pending', 'probing', 'confirmed', 'failed', 'skipped',
])

export const applicationStatusEnum = pgEnum('application_status', [
  'saved', 'applied', 'oa_received',
  'hr_screen_scheduled', 'hr_screen_completed',
  'technical_screen_scheduled', 'technical_screen_completed',
  'hiring_manager_scheduled', 'hiring_manager_completed',
  'final_round_scheduled', 'final_round_completed',
  'offer_received', 'offer_accepted', 'offer_declined',
  'rejected_after_oa', 'rejected_after_hr', 'rejected_after_technical',
  'rejected_after_hiring_manager', 'rejected_after_final',
  'ghosted', 'withdrawn',
])

export const rejectionStageEnum = pgEnum('rejection_stage', [
  'oa', 'hr', 'technical', 'hiring_manager', 'final', 'unknown',
])
...
export const coverLetterTypeEnum = pgEnum('cover_letter_type', [
  'cover_letter', 'recruiter_message', 'linkedin_connect', 'thank_you',
])
...
export const recruiterSourceEnum = pgEnum('recruiter_source', [
  'linkedin', 'referral', 'inbound', 'conference', 'other',
])

export const outreachStatusEnum = pgEnum('outreach_status', [
  'not_contacted', 'connected', 'messaged', 'replied', 'referred',
  'interview_scheduled', 'relationship_active', 'inactive',
])

export const outreachTypeEnum = pgEnum('outreach_type', [
  'connection_request', 'message', 'email', 'reply', 'call', 'referral',
])
...
export const outreachOutcomeEnum = pgEnum('outreach_outcome', [
  'no_response', 'replied', 'scheduled', 'referred', 'declined',
])
...
export const ingestionStageEnum = pgEnum('ingestion_stage', [
  'raw', 'normalized', 'embedded', 'scored',
])

export const skillExtractionMethodEnum = pgEnum('skill_extraction_method', [
  'taxonomy', 'hybrid',
])
...
export const discoveredCompaniesQueue = pgTable('discovered_companies_queue', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  index('dcq_status_idx').on(t.status),
  index('dcq_next_probe_idx').on(t.nextProbeAt),
])
...
export const jobMatches = pgTable('job_matches', {
  ...
  profileId: uuid('profile_id').notNull().references(() => candidateProfiles.id, { onDelete: 'cascade' }),
  // Match signals
...
export const applicationHistory = pgTable('application_history', {
  ...
  index('app_history_application_id_idx').on(t.applicationId),
])
...
export const recruiterOutreachHistory = pgTable('recruiter_outreach_history', {
  id: uuid('id').primaryKey().defaultRandom(),
  recruiterId: uuid('recruiter_id').notNull().references(() => recruiterContacts.id, { onDelete: 'cascade' }),
  ...
  index('outreach_history_recruiter_id_idx').on(t.recruiterId),
])
...
export const digests = pgTable('digests', {
  ...
  uniqueIndex('digests_user_date_idx').on(t.userId, t.digestDate),
])
...
export const companiesRelations = relations(companies, ({ one, many }) => ({
  intelligence: one(companyIntelligence, { fields: [companies.id], references: [companyIntelligence.companyId] }),
  jobs: many(jobs),
  recruiterContacts: many(recruiterContacts),
}))
